 ...
 */
fun calcStrideLength(heightInches: Float, strideAngles: MutableList<Float>) : Float
{
    ...
    for(i in maxAngles.indices)
    {
        ...
        val angleInRadians = (maxAngles[i] * PI / 180).toFloat()
 ...
 */
fun calcStrideLengthAvg(heightInches: Float, strideAngles: MutableList<Float>) : Float
{
    var maxAngles: List<Float>
    maxAngles = FindLocalMax(strideAngles)
    var sum = 0f
    ...
    sum /= 39.37F
    return if (maxAngles.isNotEmpty()) sum/maxAngles.size else 0f
}
