...
import com.gaitvision.AppContext
...
actual class GaitScorer {
    ...
    /**
     * Compute AE score. Latent Mahalanobis distance ΓåÆ 0-100.
    ...
    /**
     * Compute Ridge score (severity prediction ΓåÆ 0-100).
    ...
    /**
     * Compute PCA score (latent distance to normal centroid ΓåÆ 0-100).
    ...
    private fun loadJsonAsset(filename: String): String? {
        return try {
            val raw = AppContext.context!!.assets.open(filename).bufferedReader().use { it.readText() }
    ...
    private fun loadModelFile(filename: String): MappedByteBuffer? {
        return try {
            val assetFileDescriptor = AppContext.context!!.assets.openFd(filename)
